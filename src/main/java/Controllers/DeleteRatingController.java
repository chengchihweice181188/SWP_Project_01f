...
public class DeleteRatingController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeleteRatingController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeleteRatingController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int order_id;
        try {
            order_id = Integer.parseInt(orderIdParam);
        ...
        boolean updateSuccess = ratingOD.deleteFeedbackByOrderId(order_id);
        if (updateSuccess) {
            // If successful, redirect to a success page or the updated rating page
            request.setAttribute("successMessage", "Xóa đánh giá thành công.");
            request.getRequestDispatcher("viewRatingOrder.jsp").forward(request, response);
        } else {
            // If the update failed, return an error message
            request.setAttribute("errorMessage", "Xóa đánh giá thất bại !");
            request.getRequestDispatcher("viewRatingOrder.jsp").forward(request, response);
        }
    }
